      ...
      {plan.fields.map((field) => (
        <div key={field.key} className="border-b border-border-subtle py-2.5 last:border-b-0">
          <div className="flex items-baseline justify-between gap-2">
            ...
            {field.available ? (
              <span className="text-right text-base font-bold text-ink-primary [font-variant-numeric:tabular-nums]">
                {field.display_value}
              </span>
            ) : (
          ...
          {!field.available && field.unavailable_reason && (
            <div className="mt-1 text-xs leading-[1.45] text-ink-muted">{field.unavailable_reason}</div>
          )}
        </div>
      ))}

      <div className="mt-3 flex items-start gap-1.5 rounded-lg bg-surface-muted p-[8px_10px] text-xs leading-[1.45] text-ink-secondary [&_svg]:mt-px [&_svg]:h-3 [&_svg]:w-3 [&_svg]:shrink-0 [&_svg]:text-ink-muted">
        <Icon name="info" />
        <span>
